...
class PleerSource(RB.Source):
	...
	def do_get_status(self, status, progress_text, progress):
		...
		if hasattr(self, 'downloading') and self.downloading:
			...
			if self.search.is_complete():
				...
				displayedTracks = self.search.search_total if int(self.tracksperpage_value) > self.search.search_total else self.tracksperpage_value
	...
	def on_search_button_clicked(self, button):
		...
		self.tracksperpage_value = 20 if len(self.props.settings.get_string('tracksperpage-value')) == 0 else self.props.settings.get_string('tracksperpage-value')
		
		if entry.get_text():
			self.search = PleerSearch(entry.get_text(), self.tracksperpage_value, self.props.shell.props.db, self.props.entry_type)
